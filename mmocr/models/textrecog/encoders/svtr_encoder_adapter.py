...
class MixingBlock(BaseModule):
    ...
    def forward(self, x: torch.Tensor,HW) -> torch.Tensor:
        ...
        """
        if self.prenorm:
            x = self.norm1(x + self.drop_path(self.mixer(x,HW)))
            x = self.norm2(x + self.drop_path(self.mlp(x)))
        else:
            x = x + self.drop_path(self.mixer(self.norm1(x),HW))
...
class SVTREncoderAdapter(BaseModule):
    ...
    def __init__(self,
        ...
        self.avgpool = nn.AvgPool2d(avg_pool_k)
